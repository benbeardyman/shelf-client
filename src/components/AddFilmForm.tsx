...
const empty: FilmCreate = {
  ...
  date_watched: null, rating: null, notes: null,
}
...
export function AddFilmForm({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <form onSubmit={submit} className={styles.entryForm}>
      ...
      <input type="date" title="Date watched" value={form.date_watched ?? ''} onChange={e => set('date_watched', e.target.value)} />
      ...
      <button type="submit" disabled={saving}>{saving ? 'Saving…' : 'Add Film'}</button>
    </form>
  )
}
